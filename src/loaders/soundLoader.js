...
export default function loadSounds(k) {
  ...
  for (const name of music) {
    k.loadSound(name, `/public/assets/sounds/music/${name}.mp3`);
  }

  for (const [alias, target] of Object.entries(aliases)) {
    k.loadSound(alias, `/public/assets/sounds/music/${target}.mp3`);
  }

  for (const name of uiSounds) {
    k.loadSound(name, `/public/assets/sounds/uiSounds/${name}.mp3`);
  }

  k.loadSound("soundUI", `/public/assets/sounds/uiSounds/pickupCoin.mp3`);
  k.loadSound("chime-bell", `/public/assets/sounds/uiSounds/door_chime.mp3`);
  k.loadSound("door-open", `/public/assets/sounds/uiSounds/sliding_door.mp3`);
  k.loadSound("sparkle", '/public/assets/sounds/uiSounds/sparkle.mp3');

  for (const [char, suffixes] of Object.entries(voiceAliasMap)) {
  suffixes.forEach((voiceFileName) => {
    const filePath = `/public/assets/sounds/voices/${char}/${voiceFileName}.mp3`;
    k.loadSound(voiceFileName, filePath); // just this one
  });
  }
}
